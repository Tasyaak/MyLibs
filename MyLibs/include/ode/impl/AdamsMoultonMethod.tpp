...
typename AdamsMoultonMethod<StateType>::ReturnType AdamsMoultonMethod<StateType>::dGdy(const StateType& y, double tn, double h, double H) const
{
	...
	else if constexpr (std::is_same_v<StateType, ColumnVector<double>>)
	{
		std::size_t n = y.size();
		Matrix<double> f_prime(n);
		ColumnVector<double> f1, f2, y1 = y, y2 = y;
		...
		return Matrix<double>(n) - f_prime * (251 * h / 720);
	}
}
template<typename StateType>
void AdamsMoultonMethod<StateType>::NewtonMethod(StateType& y, const StateType& yn, double tn, double h, const StateType& f0, const StateType& f1, const StateType& f2, const StateType& f3) const
{
	...
	{
		Matrix<double> Q, R;
		ColumnVector<double> b1, b2, z;

		temp = G(y, yn, tn, h, f0, f1, f2, f3);
		do
		{
			prevY = y;
			std::tie(Q, R) = dGdy(y, tn, h).QRdecomposition(eps);
			Q = ~Q;

			b1 = R * y - Q * temp;
			z = GaussMethod<double>::backSubstitution(R, b1, eps);

			b2 = R * z - Q * G(z, yn, tn, h, f0, f1, f2, f3);
			y = GaussMethod<double>::backSubstitution(R, b2, eps);
			temp = G(y, yn, tn, h, f0, f1, f2, f3);
		} while (norm2(y - prevY) >= eps || temp.norm2() >= eps);
	}
}
...
template class AdamsMoultonMethod<ColumnVector<double>>;